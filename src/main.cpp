...
int main(int argc, char *argv[]) {
  ...
  options_description.add_options()(
      "vertices_file_path",
      boost::program_options::value<std::string>(&vertices_file_path)
          ->required(),
      "vertices_file_path")(
      "edges_file_path",
      boost::program_options::value<std::string>(&edges_file_path)->required(),
      "edges_file_path")(
      "start_date",
      boost::program_options::value<std::string>(&start_date)->required(),
      "start_date (YYYY-MM-DD)")(
      "end_date",
      boost::program_options::value<std::string>(&end_date)->required(),
      "end_date (YYYY-MM-DD)");
  ...
  boost::circle_topology<> topology =
      boost::circle_topology<>(TOPOLOGY_SCALING);

  for (boost::gregorian::month_iterator month_iterator(
           boost::gregorian::from_simple_string(start_date), 1);
    ...
    for (int i = 0; i < ANIMATOR_STEPS_PER_MONTH; i++) {
      graph_animator.update_layout(graph, topology,
                                   i / ANIMATOR_STEPS_PER_MONTH);
